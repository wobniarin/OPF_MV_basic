...
### Modifications on the OPF formulation : Íngrid Munné Collado UPC Barcelona-Tech
...
def OPF_model_creator(pd, pyo, math, Time_sim, V_init_pu, House_Dem_data, PV_set, PV_Gen_data, Bus_Vnom,
                      V_statutory_lim, min_Cosphi, Inverter_S_oversized, Cable_data, Transformer_rating,
                      security_margin_current, security_margin_Transformer_S):
    ...
    if 1 == 1:
        ## Loaded from csv in OPF_model\Pyomo_OPF_data\
        # Network parameters
        model.Bus_G = pyo.Param(model.Buses, model.Buses, model.Phases_abc, model.Phases_abc,
                                within=pyo.Any)  # Network conductivity matrix
        model.Bus_B = pyo.Param(model.Buses, model.Buses, model.Phases_abc, model.Phases_abc,
                                within=pyo.Any)  # Network susceptance matrix
        model.Connectivity = pyo.Param(model.Buses, model.Buses,
                                       within=pyo.Any)  # Connectivity matrix 1 if bus k and bus i are connected (zero otherwise)
    ...
    ## Variables ##
    if 1 == 1:
        # Flex control variable (In) in kW
        if 1 == 1:
            model.P_flex = pyo.Var(model.Buses, model.Phases_abc, model.time, within=pyo.NonNegativeReals,
                                      initialize=0.001) # Active power in kW at bus k and time t in kW for flexiblity purposes
        # PV Control variables # now deactivated (In)
        if 1 == 0:
            model.P_control = pyo.Var(model.PVs, model.time, within=pyo.NonNegativeReals, bounds=(0.0, 1.0),
                                      initialize=0.001)
            model.Thanphi_control = pyo.Var(model.PVs, model.time, bounds=(0.0, math.tan(math.acos(min_Cosphi))),
                                            initialize=0.001)
        ...
        if 1 == 1:
            ...
            model.Vre = pyo.Var(model.Buses, model.Phases_abc, model.time,
                                initialize=init_Vre_rule)  # Real and imaginary phase voltages
        ...
        if 1 == 1:
            ...
            # Dummy variables
        model.Dummy = pyo.Var(initialize=0.0,
                              within=pyo.NonNegativeReals)  # Dummy variable just to have something to put in the objective function
    ...
    if 1 == 1:
        ...
        if 1 == 1:
            def PV_P_rule(model, pv, t):
                return PV_Gen_data[pv, 'Profile'].loc[t]

            model.PV_P = pyo.Expression(model.PVs, model.time,
                                        rule=PV_P_rule)  # Active power generation at each bus from PVs
            #SUPPRESSED BY IN
            # def PV_Q_rule(model, pv, t):
            #     return model.PV_P[pv, t] * -model.Thanphi_control[pv, t]
            #
            # model.PV_Q = pyo.Expression(model.PVs, model.time,
            #                             rule=PV_Q_rule)  # Reactive power generation at each bus from PVs

        # Power injection per node
        if 1 == 1:
            # Active Power demand ZIP
            def ZIP_P_rule(model, k, s, t):
                return sum(House_Dem_data[h, 'P_Profile'].loc[t] * (
                            House_Dem_data[h, 'ZIP_P'].loc[t, 'P'] + House_Dem_data[h, 'ZIP_P'].loc[t, 'I'] * (
                                (pyo.sqrt(model.V_mag_2[k, s, t])) / model.Load_Vnom[h]) +
                            House_Dem_data[h, 'ZIP_P'].loc[t, 'Z'] * (model.V_mag_2[k, s, t]) / (
                                        model.Load_Vnom[h] ** 2)) for h in model.Loads if
                           (model.Load_bus_conn[h] == k and model.Load_phase_conn[h] == s))
            ...
            def ZIP_Q_rule(model, k, s, t):
                return sum(House_Dem_data[h, 'Q_Profile'].loc[t] * (
                            House_Dem_data[h, 'ZIP_Q'].loc[t, 'P'] + House_Dem_data[h, 'ZIP_Q'].loc[t, 'I'] * (
                                (pyo.sqrt(model.V_mag_2[k, s, t])) / model.Load_Vnom[h]) +
                            House_Dem_data[h, 'ZIP_Q'].loc[t, 'Z'] * (model.V_mag_2[k, s, t]) / (
                                        model.Load_Vnom[h] ** 2)) for h in model.Loads if
                           (model.Load_bus_conn[h] == k and model.Load_phase_conn[h] == s))
            ...
            def PGen_rule(model, k, s, t):
                return sum(model.PV_P[pv, t] for pv in model.PVs if
                           (model.Load_bus_conn[pv] == k and model.Load_phase_conn[pv] == s)) + model.P_flex[k,s,t]

            model.PGen = pyo.Expression(model.Buses, model.Phases_abc, model.time, rule=PGen_rule)

            # def QGen_rule(model, k, s, t):
            #     return sum(model.PV_Q[pv, t] for pv in model.PVs if
            #                (model.Load_bus_conn[pv] == k and model.Load_phase_conn[pv] == s))
            #
        ...
        if 1 == 1:
            ...
            if 1 == 1:
                ...
                def I_sp_re_rule(model, k, s, t):  # Real specified current injection per node and phase
                    return model.Psp[k, s, t] == model.Vre[k, s, t] * model.Isp_re[k, s, t] + model.Vim[k, s, t] * \
                           model.Isp_im[k, s, t]
                ...
                def I_sp_im_rule(model, k, s, t):  # Imaginary specified current injection per node and phase
                    return model.Qsp[k, s, t] == model.Vim[k, s, t] * model.Isp_re[k, s, t] - model.Vre[k, s, t] * \
                           model.Isp_im[k, s, t]
            ...
            if 1 == 1:
                ...
                def I_calc_re_rule(model, k, s, t):  # Real calculated current injection
                    return sum(sum(
                        model.Bus_G[k, i, s, a] * model.Vre[i, a, t] - model.Bus_B[k, i, s, a] * model.Vim[i, a, t] for
                        a in model.Phases_abc) for i in model.Buses if model.Connectivity[k, i] == 1)
                ...
                def I_calc_im_rule(model, k, s, t):  # Imaginary calculated current injection
                    return sum(sum(
                        model.Bus_G[k, i, s, a] * model.Vim[i, a, t] + model.Bus_B[k, i, s, a] * model.Vre[i, a, t] for
                        a in model.Phases_abc) for i in model.Buses if model.Connectivity[k, i] == 1)
        ...
        if 1 == 1:
            def I_flow_re_rule(model, l, s, t):  # Real line flows
                return sum(model.Bus_G[model.Lines_k[l], model.Lines_i[l], s, a] * (
                            model.Vre[model.Lines_i[l], a, t] - model.Vre[model.Lines_k[l], a, t]) - model.Bus_B[
                               model.Lines_k[l], model.Lines_i[l], s, a] * (
                                       model.Vim[model.Lines_i[l], a, t] - model.Vim[model.Lines_k[l], a, t]) for a in
                           model.Phases_abc)
            ...
            def I_flow_im_rule(model, l, s, t):  # Imaginary line flows
                return sum(model.Bus_B[model.Lines_k[l], model.Lines_i[l], s, a] * (
                            model.Vre[model.Lines_i[l], a, t] - model.Vre[model.Lines_k[l], a, t]) + model.Bus_G[
                               model.Lines_k[l], model.Lines_i[l], s, a] * (
                                       model.Vim[model.Lines_i[l], a, t] - model.Vim[model.Lines_k[l], a, t]) for a in
                           model.Phases_abc)
        ...
        if 1 == 1:
            def P_flow_sending_rule(model, l, s, t):  # P sending flow
                return model.Vre[model.Lines_k[l], s, t] * model.Iflow_re[l, s, t] + model.Vim[model.Lines_k[l], s, t] * \
                       model.Iflow_im[l, s, t]
            ...
            def P_flow_receiving_rule(model, l, s, t):  # P receiving flow
                return -1.0 * (model.Vre[model.Lines_i[l], s, t] * model.Iflow_re[l, s, t] + model.Vim[
                    model.Lines_i[l], s, t] * model.Iflow_im[l, s, t])

            model.P_flow_receiving = pyo.Expression(model.Lines, model.Phases_abc, model.time,
                                                    rule=P_flow_receiving_rule)

            def Q_flow_sending_rule(model, l, s, t):  # Q sending flow
                return model.Vim[model.Lines_k[l], s, t] * model.Iflow_re[l, s, t] - model.Vre[model.Lines_k[l], s, t] * \
                       model.Iflow_im[l, s, t]
            ...
            def Q_flow_receiving_rule(model, l, s, t):  # Q receiving flow
                return -1.0 * (model.Vim[model.Lines_i[l], s, t] * model.Iflow_re[l, s, t] - model.Vre[
                    model.Lines_i[l], s, t] * model.Iflow_im[l, s, t])

            model.Q_flow_receiving = pyo.Expression(model.Lines, model.Phases_abc, model.time,
                                                    rule=Q_flow_receiving_rule)
        # Losses
    ...
    if 1 == 1:
        ...
        model.Slack_im_con = pyo.Constraint(model.Buses, model.Phases_abc, model.time,
                                            rule=Slack_im_rule)  # Imaginary part
    ...
    if 1 == 1:
        ...
        if 1 == 0:
            def V_limits_rule(model, k, s, t):
                if k == 'slack':
                    ...
                    return (
                    (V_nom * V_statutory_lim[0]) ** 2, model.V_mag_2[k, s, t], (V_nom * V_statutory_lim[1]) ** 2)
        ...
        if 1 == 0:
            def Line_I_limits_rule(model, l, s, t):
                return model.Iflow_re[l, s, t] ** 2 + model.Iflow_im[l, s, t] ** 2 <= (
                            security_margin_current * Cable_data.loc[model.Lines_cable[l], 'Current rating [A]']) ** 2
        ...
        if 1 == 0:
            def Transformer_limit_rule(model, t):
                return sum(sum(
                    (model.P_flow_sending[l, s, t] ** 2 + model.Q_flow_sending[l, s, t] ** 2) for s in model.Phases_abc)
                           for l in model.Lines if model.Lines_k[l] == 'secondary') <= (
                                   security_margin_Transformer_S * Transformer_rating * 1000) ** 2
    ...
    if 1 == 0:  # I recommend using a different NLP solver than IPOPT if incorporating this constraint (e.g. knitro or modifiying IPOPT options) - otherwise, relax the constraint
        def PV_inverter_limit_rule(model, pv, t):
            ...
            if max_S > PV_Gen_data[pv, 'Rating'] * 1000 * Inverter_S_oversized:
                return (model.PV_P[pv, t] ** 2 + model.PV_Q[pv, t] ** 2) <= (
                            PV_Gen_data[pv, 'Rating'] * 1000 * Inverter_S_oversized) ** 2
            else:
                return pyo.Constraint.Skip
    ...
    #         return sum(sum((-1e3 * model.P_control[pv, t] + model.Thanphi_control[pv, t]) for pv in model.PVs) for t in
    #                    model.time)
    if 1 == 1:
        def Objective_rule(model):
                return sum(sum(sum((model.P_flex[k, s, t]) for k in model.Buses) for s in model.Phases_abc) for t in
                       model.time)
