...
Feeder_names = ['Feeder2'] # ['Feeder2'] # ['Feeder1','Feeder2','Feeder3'] - can use all three or 1 or 2
...
Time_end = 1440 #1440     # Last time step <=1440
...
from Main_packages.OPF_model_creator_v02_FLEX import OPF_model_creator
...
OPF_Pflex = np.zeros([Bus_set_no_slack.size,3, Time_sim.size]) # added by Ingrid
...
for t in Time_sim:
    ...
    for i_pv in range(len(PV_set)):
        ...
        # OPF_PV_Q[i_pv,count_t] = instance.PV_Q[PV_set[i_pv],t].expr() (IN COMMENTED)

    # P_flex output
    for i_bus in range(len(Bus_set_no_slack)):
        for phase in range(3):
            OPF_Pflex[i_bus, phase, count_t] = instance.P_flex[Bus_set_no_slack[i_bus], phase + 1, t].value
    count_t+=1
...
for t in Time_sim:
    ...
    for PV in PV_set:
        ...
        # dssCircuit.Loads.kvar = -1 * instance.PV_Q[PV,t].expr()/1000.0 (IN COMMENTED)
